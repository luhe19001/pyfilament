"""Web analyst (Claude) — a minimal, real LLM pipeline on pyfilament.

Give it URLs; it fetches each page with `curl`, then asks Claude for a structured
...
adds to a plain model call.
    ...
    └─ summarize(url, html) → PageBrief  (the Claude call, retried + cached)

The OpenAI and Gemini twins are examples/web_analyst_openai.py and
...
running, and `alembic upgrade head` once. Plus:
    ...
    python examples/web_analyst_anthropic.py
    python examples/web_analyst_anthropic.py https://www.python.org https://news.ycombinator.com
"""
...
# Haiku 4.5 is Anthropic's fastest, most cost-effective model — and it supports
# structured outputs. Override with ANTHROPIC_MODEL for a more capable run.
...
async def brief(url: str, html: str, model: str) -> PageBrief:
    # messages.parse enforces the PageBrief schema (field names AND types) and
    # hands back the validated object on .parsed_output.
    resp = await client.messages.parse(
        model=model,
        max_tokens=4096,
        system=_SYSTEM,
        messages=[{"role": "user", "content": _prompt(url, html)}],
        output_format=PageBrief,
    )
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize web pages with Claude + filament.")
