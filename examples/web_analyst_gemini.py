"""Web analyst (Gemini) — a minimal, real LLM pipeline on pyfilament.

Give it URLs; it fetches each page with `curl`, then asks Gemini for a structured
brief (category, title, summary, key points, audience). The fetch → summarize tree
is durable, retried, cached, and visible in the dashboard — that's what filament
adds to a plain model call.

    analyze(url)
    ├─ fetch_page(url)      → HTML       (cached + retried + rate-limited)
    └─ summarize(url, html) → PageBrief  (the Gemini call, retried + cached)

The OpenAI and Claude twins are web_analyst_openai.py / web_analyst_anthropic.py —
same pipeline, same PageBrief, different SDK in one function.
...
running, and `alembic upgrade head` once. Plus:
    ...
    python examples/web_analyst_gemini.py
    python examples/web_analyst_gemini.py https://www.python.org https://news.ycombinator.com
"""
...
# Flash-Lite is Gemini's fastest, most cost-effective tier. Override with GEMINI_MODEL.
MODEL = os.getenv("GEMINI_MODEL", "gemini-3.1-flash-lite")

client = genai.Client(api_key=os.environ["GEMINI_API_KEY"])  # AI Studio key
...
async def brief(url: str, html: str, model: str) -> PageBrief:
    # Ask for JSON constrained to the PageBrief schema, then validate it. (google-genai
    # has no agent framework, so this is a single structured-output call.)
    resp = await client.aio.models.generate_content(
        model=model,
        contents=_prompt(url, html),
        config=types.GenerateContentConfig(
            system_instruction=_SYSTEM,
            response_mime_type="application/json",
            response_json_schema=PageBrief.model_json_schema(),
        ),
    )
    return PageBrief.model_validate_json(resp.text)
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize web pages with Gemini + filament.")
